...
public:
   ...
   void deleteSmallest();
     //Function to delete smallest item from list
     //Postcondition: As the smallest item will be the first item in the
     //               list, that will be deleted and the 'first' pointer
     //               will be moved down to the next item.
...
void orderedLinkedList<Type>::deleteSmallest()
{
   ...
   else
   {
      ...
      this->first = this->first->link;
      this->count--;
      if(this->first == NULL) //Case 2: List only has one node
      {
         this->last = NULL;
      }
      delete current;
